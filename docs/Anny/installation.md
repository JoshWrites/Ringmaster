...
> **Pinning a version:** `pip install .` installs from whatever commit you cloned. Once tagged releases exist, you can pin to a specific version: `pip install git+https://github.com/JoshWrites/Ringmaster.git@v0.1.0`
...
Edit `~/.config/systemd/user/ringmaster.service` to point `ExecStart` at your venv's Python. The full file should look like this:
